...
from moderatorAdmin.serializer import  CategoryBlogUpSerializer,BlogCategorySerializer, CategorySerializerA,ManyCategorySerializer, CommentSerializer, RoleSerializer,UserRoleSerializer,BlogSerializer, UserSerializer, MagazineSerializer,CategorySerializer,UserSerializerB,CategorySerializer,FeedbackSerializerUp
...
def postFeedback(request):
    ...
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            serializer.save()
            notifyUser(api_endpoint_url, pk, user_id, receiver=receiver, type="feedback")
...
@csrf_exempt
@api_view(['POST'])
def addCategoryToBlog(request):
        try:
            blog = Blog.objects.get(id=request.data.get('id'))
            #category = Category.objects.get(id=request.data.get('category_id'))
            #print("XXXXXX", category.name)
        except Blog.DoesNotExist:
            return Response({"error": "Blog does not exist"}, status=status.HTTP_404_NOT_FOUND)
        except Category.DoesNotExist:
            return Response({"error": "Category does not exist"}, status=status.HTTP_404_NOT_FOUND)
        if isinstance(request.data.get('category_id'), int): 
            #print("categoooory", request.data.get('category_id'))
            #category = Category.objects.get(id=request.data.get('category_id'))
            #blog.categories.add(category)
            category_id = request.data.get('category_id')
            print("LAAAAAAAAAAST---", category_id)
            category = Category.objects.get(id=category_id)
            blog.categories.add(category)
            #blog.categories.add(category)


        elif isinstance(request.data.get('category_id'), list):  # Check if it's a list of IDs
            for category_id in request.data.get('category_id'):
                try:
                    category = Category.objects.get(id=category_id)
                    blog.categories.add(category)
        ...
        #print("xxxx",blog.categ)
        #blog.categories.add(category)
        #print("yyyyy", blog)
        #print("xxxxxx", blog.categories)
        serializer = BlogCategorySerializer(blog)
...
def approvePost(request):
    ...
    if serializer.is_valid():
        serializer.save()
        notifyUser(api_endpoint_url, pk, user_id, receiver=receiver, type="blog-approval")
        return Response(serializer.data)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def rejectPost(request):
    ...
    try:
        ...
        print("RECEIVERRRRRRRRR", receiver)
        
    except Blog.DoesNotExist:
        return Response({"error": "DELETED"})

    serializer =BlogSerializer(instance, data=request.data)
    print("WHATTSS THAT" , instance.rejection_number)
    print("REQUEEEEEEEEST", serializer.initial_data)
    if serializer.is_valid():
        if(instance.rejection_number<=1):
            serializer.save()
            notifyUser(api_endpoint_url, pk, user_id, receiver=receiver,type="blog-rejection")
        return Response(serializer.data)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def test_api(request):
    print("HELLO TEST")
    return Response("last successsssssssssssssssssss")
