    ...
    <main class="editing">
      <v-container>
        <div class="admin-table">
          ...
          <v-text-field v-model="lastNameSearch" label="Search by Last Name" class="mx-4"></v-text-field>
          ...
          >
            <template v-slot:item.action="{ item }">
              ...
              <v-btn v-if="isModeratorOrUser(item.role)" @click="changeUserRole(item.id, item.role)">Change Role</v-btn>
            </template>
          </v-data-table>
        </div>
      </v-container>
  ...
  export default {
    ...
    computed: {
      ...
      filteredUsers() {
        ...
        return this.users.filter(user => user.last_name.toUpperCase().includes(searchUpperCase));
      },
    },
    methods: {
      async getAllUsers() {
        try {
          const response = await axios.get('http://18.206.186.97:8000/getAllUsers');
